...
| **Patient count per consultant** (table) | USERS-driven (H2/B9), but lists only consultants who actually hold at least one active admission — a zero-census consultant no longer appears (the old "every on-service consultant, zeros included" behaviour was noise). Per row, over their **active** admissions: **New** (`is_new_assignment = 1`) and **Old** (`is_new_assignment = 0`). This is a **managed flag**, not a rolling time window — it is set to 1 on assign / handover / shuffle and cleared (set to 0) only on discharge or reassignment (`admissions.is_new_assignment`, §2 of DATABASE-AND-BEHAVIOR.md). A patient assigned weeks ago and never touched since is still "New"; correction from a prior "assigned within 24h" description, which does not match the code. Also: **Ward** (non-ICU), **ICU**, **TB** (any diagnosis ∈ `tb_diagnoses`), **Active** (non-ICU AND not medically-discharged AND not long-term AND not TB), **Total**. Grouped: on-service hospitalists (specialty 1) → on-service subspecialists → off-service. |
...
| **Physician drill-down donuts** (one consultant) | Over that consultant's **non-ICU** episodes closed in range (soft-deleted excluded). **"Discharge destinations"** by `transfer_type`: Discharged (`discharge from ward`) · Intra-dept transfer (`transfer to other speciality`) · **Transfer to ICU** (`other transfer` with `discharge_to = 'Intensive Care (ICU)'`) · Out-dept transfer (any other `other transfer`, including a blank destination) · ICU discharge. **"Discharged to"** by `discharge_to`: Home · Other Facility · LAMA · Absconded · Mortuary · **ICU** (`Intensive Care (ICU)`) · Transfer (anything else, including blank). **Deviation from legacy charts.php (2026-09-24, owner request):** legacy counted a ward→ICU move as "Out-dept transfer" / "Transfer"; it now has its own slice. Totals are unchanged — each donut still sums to the consultant's non-ICU closes (the first counts only the four typed transfer kinds, as before) — and `transToIcu` and every KPI are untouched. |
