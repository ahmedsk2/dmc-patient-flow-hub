...
| **Pre-deploy / incident dumps** | OCI Object Storage `dmc-db-backups`, same as the automated backups above | **Secret** | Everything | **Procedure fixed 2026-09-22** (DEPLOY-LARAVEL.md §2): the pre-deploy step now runs `db-backup.py` itself — the same encrypted, off-box pipeline as the nightly backup — instead of a manual `mysqldump \| gzip` into the operator's home directory, so no *new* unencrypted dump is written to the host at any point in a deploy. **Closed 2026-09-22.** The old manual procedure (used between 2026-09-03 and this fix) wrote `~/dmc_demo_pre-deploy_*.sql.gz` into the operator's home directory; a read-only sweep of the host that day (`/home/ubuntu`, `/root` and `/var/backups/dmc` for `*.sql`, `*.sql.gz` and `*pre-deploy*`) found **no** DMC dump — the only hit was an empty file belonging to an unrelated project — and `/var/backups/dmc` held only the encrypted `.enc` copies |
| **Legacy database dumps** (`dmc_prod` exports for `legacy:import`, plus the live legacy DB behind `dmc-im.com`) | **Inventory (2026-09-03):** live legacy DB on SiteGround (US); developer workstation `Downloads/`: 7× `dbqeqbacgfvmhk*.sql` + `dmc_laravel_export.sql(.gz)` + `_recon.sql`; OCI host `/home/ubuntu/migrate/dmc/`: plaintext `dmc_demo.sql.gz` + a pre-refresh dump + leftover key files — **shredded 2026-09-03, re-checked 2026-09-22 (scripts only)** | **Secret** | Real PHI | Encrypt or destroy each copy on a schedule — **open action for the workstation copies and the SiteGround terms** (the host copies are done), see DATA-RETENTION.md and EVIDENCE-PACK G8 |
...
| **Audit-log exports** (`/audit/export`, `/audit/export-xlsx`) | Staff device | **Secret** | Same as above | Admin only; **audited since 2026-09-03** (`AuditController::logExport()` writes `audit.export.csv` / `audit.export.xlsx` via `App\Support\Audit`) — delete after use |
...
| **Statistics exports** (XLSX/PDF) | Staff device | **Confidential** unless a cell identifies an individual → **Secret** | Aggregates, consultant names | Admin only; **audited since 2026-09-03** (`StatisticsController::logStatisticsExport()` writes `statistics.export.xlsx` / `statistics.export.pdf`) |
...
| Storage | Encrypted at rest; keys separate from data; in-Kingdom only | **GAP for the MySQL volume (planned)** — OCI block/boot encryption today, no application-level column encryption for structured PHI; audit archive in-Kingdom; **backups and hourly binlog shipping are encrypted, off-box and in-Kingdom (live since 2026-09-03), key held separately from the host** (BACKUP-AND-RESTORE.md §2.2, §10) | [VERIFY] |
