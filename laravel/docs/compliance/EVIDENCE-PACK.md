...
| G14 | ~~Laravel session cookie lacks the `__Host-` prefix~~ **CLOSED in code 2026-09-03**: `config/session.php` prefixes the cookie with `__Host-` whenever `SESSION_SECURE_COOKIE` is true (production); takes effect at the next deploy and signs everyone out once | `evidence/sec-web-2026-09-03.md` SPC-WEB-001 | engineering | deploy |
