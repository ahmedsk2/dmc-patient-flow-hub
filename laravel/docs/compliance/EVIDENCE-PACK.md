...
| P8 | Security of processing: organisational and lifecycle measures | | CI gates: gitleaks, Semgrep, `composer audit` gate, `npm audit`, Vitest/PHPUnit (`.github/workflows/laravel-ci.yml`, `docs/CI.md`); GitHub secret scanning + push protection + Dependabot; SHA-pinned actions; `RELEASE-CHECKLIST.md` | `gh run list --workflow=laravel-ci.yml`; GitHub → Settings → Code security (screenshot) | IMPLEMENTED |
...
|---|---|---|---|---|
| G1 | Statistics exports, audit-log exports and report PDFs write no audit row (registry exports do) | `StatisticsController`, `AuditController`, `ReportsController` have no `Audit::log` call | engineering | |
| G2 | Exports and printed sheets carry no classification label | `DATA-CLASSIFICATION.md` §4 | engineering | |
...
| G14 | Laravel session cookie lacks the `__Host-` prefix (low; all prerequisites — Secure, `path=/`, no Domain — already hold) | `evidence/sec-web-2026-09-03.md` SPC-WEB-001 | engineering: set `SESSION_COOKIE=__Host-dmc-internal-medicine-session` in the Coolify runtime env, restart | |
| G16 | **Production-readiness re-score 2026-09-03: BLOCKED 58/100** (emphasis 70; 2 Critical, 19 High, 47 intake). Criticals: no required status checks on `main` + manual smoke/rollback. Highs itemised in the report; the top seven fixes are ranked there. | [`evidence/prod-ready-2026-09-03.md`](evidence/prod-ready-2026-09-03.md) | engineering + owner | re-score after Top fixes 1–5 |
