...
| P19 | Destruction / secure disposal | | Soft deletes + admin hard delete with step-up and audit row; `DATA-RETENTION.md` §5; bucket lifecycle rules; legacy dump shredding (`BACKUP-AND-RESTORE.md` §2.7); workstation clean-up inventory, 38 files hashed, recycled and destroyed (bin emptied by the owner) 2026-09-24 ([`evidence/workstation-phi-cleanup-2026-09-24.md`](evidence/workstation-phi-cleanup-2026-09-24.md)) | Owner deletes the last 13 Coolify dumps once the bucket's 14-day rule releases them (2026-10-07) and empties the bin of their 53 recycled laptop copies; retention decision for the 2026-07-19 boot-volume backup | PARTIAL |
...
| G17 | **Risk accepted (waiver, expires 2026-12-03): no second reviewer on `main`** (prod-ready SEC-11 / CICD-11). One engineer maintains the repository, so a required second approval would block every merge. Compensating controls: four required admin-enforced CI checks, SHA-pinned actions, CODEOWNERS, PR-only `main` with no force-push, operator-triggered dumped-first deploys with a rehearsed 50-second rollback, hash-chained audit log. Recorded in [`../../.prod-ready/waivers.yml`](../../.prod-ready/waivers.yml) (change-controlled, the only committed file under `.prod-ready/`); revisit when a second engineer joins or at the next readiness review | owner decision 2026-09-03 | owner | 2026-12-03 |
