...
| C6 | Migration dumps and exports | Operator workstations, the OCI host under `/home/ubuntu/migrate/dmc/` (CONFIRMED-FACTS D1) | Operator company | Manual. Host copies shredded 2026-09-03; the workstation's DMC files inventoried and destroyed 2026-09-24 (hashed, recycled, bin emptied by the owner — [`evidence/workstation-phi-cleanup-2026-09-24.md`](evidence/workstation-phi-cleanup-2026-09-24.md)) — the laptop's real-data WAMP databases dropped by the owner 2026-09-24 (verified) |
| C7 | The `dmc-im.com` mailbox | SiteGround, **United States** | Operator company | Mailbox policy [VERIFY] (§2.6) |
| C8 | Coolify's own database dumps of `dmc_demo` — **not client-side encrypted**; no new ones since 2026-09-24 | In-Kingdom OCI bucket `coolify-backups` (shared with the host's other apps), the host's Coolify backup folder, and the owner's workstation mirror | Operator company; Oracle | Bucket: **none** — no lifecycle expiry, only a 14-day retention rule; host: Coolify keeps 14 (§2.3) |
| C9 | Whole-disk boot-volume backups of the host (every app's data) | OCI `me-riyadh-1` | Operator company; Oracle | OCI backup policy `weekly-4` expires them after four weeks; one manual full backup (2026-07-19) has **no expiry** (§2.3) |
...
| Coolify's own dumps of `dmc_demo` (plain SQL) | **Laravel (C8)** — in-Kingdom OCI bucket `coolify-backups`, the host's Coolify backup folder, the owner's local mirror | Coolify's scheduled backup of the shared MySQL — **`dmc_demo` taken out of it 2026-09-24** (owner decision), so no new copies | Bucket: **none set** — deleted by hand: 53 of the 66 on 2026-09-24, the last 13 once the 14-day retention rule releases them (2026-10-07; REMAINING-WORK); host: Coolify keeps the last 14 executions | 13 in the bucket (2026-09-10 → 09-23), 7 on the host (verified 2026-09-24) | **GAP** — no lifecycle rule; Coolify's own S3 retention has not pruned them | Object deletion in the bucket; the host copies age out; the local mirror follows the bucket |
| Boot-volume backups of the host | **Laravel (C9)** — OCI `me-riyadh-1` | OCI volume backup policy `weekly-4`, weekly | Four weeks (policy); the manual full backup of 2026-07-19 has **no expiry** — owner decision | Four scheduled + one manual (verified 2026-09-24) | OCI backup policy | Scheduled expiry; delete the manual backup in the OCI console once decided |
...
| `trusted_devices` | `expires_at` (fixed window `settings.mfa_trusted_device_hours`, default twenty-four hours; never extended) or `revoked_at` | **Proposed: delete ninety days after expiry/revocation** [PLACEHOLDER] (the row is kept briefly as an audit trail of device trust; `mfa.device_trusted`/`mfa.device_revoked` audit rows remain the permanent record) | **Rows past `expires_at` are deleted daily** (R11, `auth:prune-expired`, 2026-09-22 — confirmed the feature is still live and separate from the disabled login "remember me" cookie before building this). A row that is `revoked_at` but not yet past its own fixed window is deliberately left alone — a "definitely expired" sweep, not the ninety-day grace period above | **Built for the expiry half.** The ninety-day-after-revocation grace window is still **GAP** — a deliberate scope cut, [PLACEHOLDER] if the hospital wants it | SQL delete |
| `pending_registrations` | `expires_at` (**thirty minutes** after creation) | Delete at expiry | **Done 2026-09-22** — swept daily by `auth:prune-expired` (`routes/console.php`, 03:15), closing the "no sweep present" gap this row used to flag; a row past `expires_at` (still holding its plaintext recovery-code set) is deleted | `auth:prune-expired`, scheduled daily | SQL delete |
| `password_reset_tokens` | `created_at` | Token validity per `config/auth.php` (`passwords.users.expire` = sixty minutes) | **Done 2026-09-22** — swept daily by `auth:prune-expired`, using the same `passwords.users.expire` cutoff `auth:clear-resets` would | `auth:prune-expired` reuses the framework's expiry config rather than shelling out to `auth:clear-resets`, so one run reports all three tables' counts together (one audit row, counts only) | SQL delete |
...
| 3 | Build `records:prune` (dry-run default, `--confirm`, audited, refuses when the period is unset) or the anonymiser | **Scaffolded 2026-09-22 (CMP-02):** `records:retention-report --years=N` exists — READ-ONLY, prints COUNTS only (admissions whose episode closed more than N years ago; patients whose every episode is that old), refuses to run without `--years`. It has **no delete or anonymise mode at all**; its own output says so. The destruction/anonymisation mechanism itself is still **GAP** — blocked on item #1 (the legal retention period) and item #2 (DPO + clinical owner: delete vs. anonymise) | [PLACEHOLDER] | 2026-09-22 |
...
| 10 | Nightly sweeps: expired sessions (file driver), `pending_registrations`, `trusted_devices` (after ninety days), `auth:clear-resets` | **Done for `pending_registrations`, `trusted_devices` (expiry only, not the ninety-day grace) and `password_reset_tokens`** — R11, `auth:prune-expired`, scheduled daily 03:15 (§2.4). Expired-sessions sweep is now moot: production runs `SESSION_DRIVER=database` (CLAUDE.md §10, since 2026-09-03), so file-driver GC doesn't apply; a `sessions`-table purge would still be **GAP** if the driver ever reverts to `file` | [PLACEHOLDER] | 2026-09-22 |
