...
| C8 | Coolify's own daily database dumps — **not client-side encrypted** | In-Kingdom OCI bucket `coolify-backups` (shared with the host's other apps), mirrored daily to the owner's workstation | Operator company; Oracle | **None** — no lifecycle expiry, only a 14-day retention rule (§2.3) |
...
| Coolify's own daily dumps of `dmc_demo` (plain SQL) | **Laravel (C8)** — in-Kingdom OCI bucket `coolify-backups`, plus the owner's local mirror | Coolify's scheduled backup of the shared MySQL | **None set** — kept indefinitely today; owner decision (REMAINING-WORK): take `dmc_demo` out of that job, or apply the C4 period | 66 daily dumps since 2026-07-19 (verified 2026-09-24) | **GAP** — no lifecycle rule; the 14-day retention rule only blocks early deletion | Bucket lifecycle rule or object deletion; the local mirror follows the bucket |
