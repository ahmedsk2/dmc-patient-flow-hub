...
| Database backups (the DMC pipeline is encrypted; the Coolify and boot-volume copies are not client-side encrypted) | OCI Object Storage bucket `dmc-db-backups`, **in-Kingdom**; local encrypted copies in `/var/backups/dmc/`; OCI boot-volume backups of the whole host disk (weekly, 4 weeks, Riyadh; a manual full backup of 2026-07-19 without expiry was deleted 2026-09-24); Coolify's own **unencrypted** dumps in the Riyadh bucket `coolify-backups` (daily until 2026-09-24, when `dmc_demo` was taken out of that job; 53 deleted 2026-09-24, the last 13 locked until 2026-10-07); the owner's local copy (the pipeline's encrypted objects, daily since 2026-09-24, kept 90 days; plus a mirror of that bucket; owner decision 2026-09-24) | Sub-processor (storage) | Everything in the **Laravel** database | Nightly encrypted off-box dump; `backup:verify` staleness alert; ninety-day bucket retention is a **placeholder pending legal** | [PLACEHOLDER] |
...
| Field | Content |
|---|---|
| **Description** | **Laravel copy:** a nightly encrypted off-box dump runs to the in-Kingdom OCI bucket `dmc-db-backups`, with encrypted local copies in `/var/backups/dmc/`, a daily `backup:verify` staleness alert and a logged restore drill ([`../BACKUP-AND-RESTORE.md`](../BACKUP-AND-RESTORE.md)). Manual `mysqldump ... \| gzip > ~/pre-deploy-*.sql.gz` is still taken on the host before each deploy and before `legacy:import`. Outside that pipeline (found 2026-09-24): Coolify's own scheduled backup wrote a daily **plain-SQL** dump of the database to the in-Kingdom bucket `coolify-backups` until it was stopped the same day (53 of its 66 old dumps deleted 2026-09-24; the last 13 locked by the bucket's 14-day rule until 2026-10-07; mirrored to the owner's workstation), and OCI backs up the host's whole boot volume weekly (four weeks; a manual full backup of 2026-07-19 without expiry was deleted 2026-09-24) — CONFIRMED-FACTS D8. **Legacy daily system:** whatever backup SiteGround takes of the shared-hosting account — not specified, not verified and not restore-tested by the processor [VERIFY — SiteGround backup scope, retention and location]. |
| **System(s) today / where** | Laravel backups: **in-Kingdom** (OCI bucket + the host itself + OCI's weekly boot-volume backups + Coolify's own unencrypted dumps in `coolify-backups`; no cross-region copy, verified 2026-09-24), plus the owner's local copy of the encrypted objects (owner decision 2026-09-24). Legacy backups: **United States**, inside SiteGround's platform. |
...
| **Security measures** | Encryption in transit and at rest for the off-box dump; separate bucket credentials; staleness alerting; a proven restore drill. **GAP:** pre-deploy dumps and the legacy-side PHI copies listed in CONFIRMED-FACTS D1 (operator workstations, `/home/ubuntu/migrate/dmc/`) are still uninventoried and partly unencrypted; the old Coolify dumps (and the owner's local mirror of them) and the boot-volume backups carry only OCI's at-rest encryption, not the pipeline's client-side encryption, and the Coolify dumps never expire (D8). |
