...
| Database backups (the DMC pipeline is encrypted; the Coolify and boot-volume copies are not client-side encrypted) | OCI Object Storage bucket `dmc-db-backups`, **in-Kingdom**; local encrypted copies in `/var/backups/dmc/`; OCI boot-volume backups of the whole host disk (weekly, 4 weeks, Riyadh; one manual full backup of 2026-07-19 without expiry); Coolify's own **unencrypted** dumps in the Riyadh bucket `coolify-backups` (daily until 2026-09-24, when `dmc_demo` was taken out of that job; 66 remain, no expiry); the owner's local copy (today a mirror of that bucket; owner decision 2026-09-24) | Sub-processor (storage) | Everything in the **Laravel** database | Nightly encrypted off-box dump; `backup:verify` staleness alert; ninety-day bucket retention is a **placeholder pending legal** | [PLACEHOLDER] |
...
| **System(s) today / where** | Laravel backups: **in-Kingdom** (OCI bucket + the host itself + OCI's weekly boot-volume backups + Coolify's own unencrypted dumps in `coolify-backups`; no cross-region copy, verified 2026-09-24), plus the owner's local copy (owner decision 2026-09-24). Legacy backups: **United States**, inside SiteGround's platform. |
