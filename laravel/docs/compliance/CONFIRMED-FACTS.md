...
| # | Fact | Value | Tag |
|---|---|---|---|
| D1 | Legacy PHI copies | (a) **Live legacy DB on SiteGround (US)** behind dmc-im.com; (b) owner workstation: the legacy dumps and exports, the local Laravel export and other DMC files — **38 files moved to the Recycle Bin on 2026-09-24** with a hashed inventory ([`evidence/workstation-phi-cleanup-2026-09-24.md`](evidence/workstation-phi-cleanup-2026-09-24.md)); emptying the bin is the owner's step; the laptop's WAMP still holds real-data databases (`dmc_laravel`, `dmc_prod`, `dmc`) for the owner to drop; (c) OCI host `/home/ubuntu/migrate/dmc/`: the plaintext dumps and leftover key files were **shredded 2026-09-03** (re-checked 2026-09-22: scripts only); (d) encrypted backups in `/var/backups/dmc/` and the bucket; (e) Coolify's own unencrypted daily dumps in `coolify-backups` and the owner's local mirror of them (see D8). | VERIFIED (gap) |
