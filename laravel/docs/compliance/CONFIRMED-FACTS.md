...
| B4 | Hosting processor — OCI | **Oracle Systems Limited** (Saudi contracting entity; Cloud Services Agreement v062223 [PROPOSED]). Company-held pay-as-you-go tenancy, Basic support, **Riyadh region `me-riyadh-1` only** (since 2026-09-23 the tenancy is also *subscribed* to `me-jeddah-1` — OCI cannot remove a subscription — with **no resources there**, and the tenancy quota `ksa-data-residency` zeroes every data-bearing service outside `me-riyadh-1`; verified 2026-09-24), Oracle-managed AES-256 volume encryption (default). Oracle's **global** support staff may access the tenancy under Oracle's policies. | CONFIRMED |
...
| # | Fact | Value | Tag |
|---|---|---|---|
| D1 | Legacy PHI copies | (a) **Live legacy DB on SiteGround (US)** behind dmc-im.com; (b) owner workstation: the legacy dumps and exports, the local Laravel export and other DMC files — **38 files hashed, moved to the Recycle Bin and destroyed on 2026-09-24** when the owner emptied the bin (verified the same day; inventory in [`evidence/workstation-phi-cleanup-2026-09-24.md`](evidence/workstation-phi-cleanup-2026-09-24.md)); the laptop's real-data WAMP databases (`dmc_laravel`, `dmc_prod`, `dmc`) were dropped by the owner on 2026-09-24 (verified); (c) OCI host `/home/ubuntu/migrate/dmc/`: the plaintext dumps and leftover key files were **shredded 2026-09-03** (re-checked 2026-09-22: scripts only); (d) encrypted backups in `/var/backups/dmc/` and the bucket; (e) Coolify's own unencrypted daily dumps in `coolify-backups` and the owner's local mirror of them (see D8). | VERIFIED (gap) |
...
| D7 | Backup geography (owner, 2026-09-24) | **Riyadh only — no second cloud region.** The owner keeps an additional local copy of the encrypted backup objects — a daily workstation sync set up the same day (only new objects, kept 90 days, refuses while the backup key is on that machine; seeded with 510 objects verified identical to the bucket). The empty Jeddah test bucket was deleted the same day. | CONFIRMED |
| D8 | Cross-region copies (verified 2026-09-24, read-only) | **None.** No replication policy on `dmc-db-backups` or `dmc-audit-log`; the host boot volume has no cross-region replica, and its backup policy `weekly-4` has no destination region; nothing exists in `me-jeddah-1`. **Boot-volume backups** (whole host disk — every app's data): weekly incremental kept 4 weeks, plus a manual full backup of 2026-07-19 with no expiry, **deleted by the owner on 2026-09-24** (verified `TERMINATED`). **Coolify's own scheduled backup** of the shared MySQL wrote a daily **unencrypted** dump of `dmc_demo` to the private Riyadh bucket `coolify-backups` until **2026-09-24, when the owner had `dmc_demo` taken out of that job**; left behind: 66 dumps in the bucket (2026-07-19 → 2026-09-23, ≈ 1.3 GB; no lifecycle expiry, only a 14-day retention rule) — **53 deleted by the owner on 2026-09-24, the 13 newest locked by that rule until 2026-10-07** — 7 on the host (aged out by Coolify's local retention) and the owner's workstation mirror of the bucket (its 53 matching copies recycled and destroyed the same day). | VERIFIED |
