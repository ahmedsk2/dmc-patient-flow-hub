...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **P0** | **[COMPANY LEGAL NAME] — the developer / operator** (PRIMARY processor) | [COMPANY LEGAL NAME, Commercial Registration No., city] — PLACEHOLDER | Builds, hosts (holds the OCI tenancy and the `towardpcc.com` domain), operates, administers and supports the hub; engages P1–P3 as its sub-processors | **Everything** — full application and database access, including all patient identifiers and clinical narratives; also holds data exports on developer workstations (see DATA-CLASSIFICATION.md) | Patients, staff | Primarily in-Kingdom (OCI Riyadh); developer-held export copies wherever the company operates | Depends on where the company / its staff operate [VERIFY] | **NO — none yet. TOP ACTION (A0).** | Controller–processor agreement with Implementing Regulation Art. 17 minimum content [PROPOSED] | Yes, if company staff access from outside the Kingdom | DPO + owner | **OPEN — highest priority** |
| P1 | **Oracle Cloud Infrastructure (OCI)** | Oracle Systems Limited (the Saudi contracting entity; Oracle Cloud Services Agreement v062223) [PROPOSED]; HQ United States | Compute, block storage, database host, backups, audit-archive object storage | Everything: patient identifiers, admission episodes, consultations, handover notes, staff accounts, audit log, backups | Patients, staff | Riyadh region, Saudi Arabia (in-Kingdom) | Storage/processing in-Kingdom (Riyadh); **but** Oracle's **global** support staff may access the tenancy from abroad under Oracle policies [PROPOSED] | **No signed DPA on file** (company-held PAYG tenancy, Basic support) | Oracle DPA + Data Transfer Annex Art. 5 §I / Exhibit G Saudi SCCs for any non-adequate onward transfer [PROPOSED] | Yes — for the support-access position | company (operator) + DPO | OPEN |
| P2 | **Cloudflare** | Cloudflare, Inc.; HQ United States | DNS, reverse proxy / WAF, TLS termination at the edge | Decrypted HTTP traffic **in transit**: page payloads that can contain patient identifiers and clinical fields, session cookies, staff identities. Request metadata (IP, URL, headers, timing) in Cloudflare logs/analytics. Patient identifiers are never in URLs by design, so metadata carries no PHI. | Patients (via page content), staff | Cloudflare edge; a Saudi PoP (Riyadh/Jeddah/Dammam) may serve it but **not contractually guaranteed on the Free plan** | **Yes** — a non-Saudi processor decrypts the traffic; on the **Free plan** edge location is not fixed and **Regional Services (in-Kingdom TLS termination) is unavailable** — it is Enterprise-only [PROPOSED] | Standard self-serve Customer DPA v6.4, applied by reference; not negotiable on Free [PROPOSED] | Requires a decision — see §5.2 | **Yes** — completed in §5.2 (template) | DPO + IT lead | OPEN |
| P3 | **Transactional SMTP relay** | **SiteGround** — the `dmc-im.com` mailbox (`mail.dmc-im.com`, on Google Cloud); **United States**; company-held account [PROPOSED — confirm SiteGround contracting entity] | Outbound email: one-time codes, password-reset links, username reminders, monthly statistics PDF | Staff email addresses and names, OTP codes, signed reset links, **aggregate** monthly figures (no patient-level data by design). Provider may retain message logs / bodies per its own policy [VERIFY]. | Staff only | United States | **Yes** (staff personal data) | **No DPA on file** | SDAIA Standard Contractual Clauses + a Transfer-Regulation Art. 7 risk assessment [PROPOSED] — or move to an in-Kingdom relay (§5.3) | Yes (Art. 7(1)(A) if SCCs are relied on) | IT lead | OPEN |
...
> **Ground truth (2026-09-03, owner-confirmed).** The hub is **built and operated by a developer
> company (P0)**, not by the hospital's own IT — the company holds the OCI tenancy, the domain and
> full data access. **No controller–processor contract exists yet**, so **action A0 (sign that
> contract, with IR Art. 17 minimum content) is the single highest priority in this document.** The
> "Owner" column below names who drives each item, but the hospital (controller) remains accountable
> for all of it. Separately, note that the **current daily production system is still the legacy PHP
> app on `dmc-im.com` (SiteGround, US)** — this register describes the go-forward Laravel hub; the
> legacy system's own hosting (SiteGround, US) is an additional processing arrangement that this
> register must also cover before cutover.
...
| Exporter / importer | Exporter: Dammam Medical Complex, Eastern Health Cluster (Saudi Health Holding Company) (controller). Importer: Cloudflare, Inc. (processor), United States, operating a global network. |
