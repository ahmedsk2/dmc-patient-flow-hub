...
| **Existing controls** | Registry and every export are Admin-only; `registry.search` is always audited (redacted filters), `registry.export` / `registry.export_xlsx` audited with row count; **since 2026-09-03 every other export and report PDF is audited as well** (`statistics.export.*`, `audit.export.*`, `report.pdf.*` — CONFIRMED-FACTS C12), with `report.pdf.governance` and the audit-log exports grouped under the viewer's PHI-read category so bulk reads stand out, and every download labelled `SECRET-`/`CONFIDENTIAL-` with a classification footer (C13); optional per-record open logging (`registry.open`, `handover.read` with MRN); every modification audited with before/after; handover revisions append-only; audit rows cannot be edited or deleted through the ORM and are hash-chained, verified nightly, shipped hourly to WORM storage; hard delete of an admission requires Admin + step-up and is audited. |
| **Residual** | L3 × S3 = **9 Medium** for the Laravel copy — the export blind spot that previously sat inside this score is closed, but detection still depends on someone reviewing the log, and `log_record_opens` is OFF. On the legacy side the residual stays at the inherent **16 High**: nothing there records a read, an export or an edit. |
| **Planned** | Turn `log_record_opens` ON [PLACEHOLDER — decision]; periodic review of `registry.export` and the new `report.pdf.*` / `*.export.*` rows by the DPO [PLACEHOLDER]; staff confidentiality undertaking and training [PLACEHOLDER — HR]; **for the legacy daily system there is nothing to review — only cutover closes it** (R13). |
...
| **Existing controls** | Laravel side: URLs PHI-free (so web/CDN/CSP logs are clean); CSP reports carry no PHI; `redactedFilters` on registry search audit; **every export and report PDF audited and labelled since 2026-09-03** (C12/C13), so a file that reaches a staff device is at least registered and carries its classification on the file name and, for PDFs, in a footer. **None of this constrains the legacy daily system**, whose URLs, logs and exports have never been inventoried [VERIFY], nor the migration dumps (D1). |
...
| 8 | Audit the statistics and audit-log exports; decide default for `log_record_opens` | R6, R12 | [PLACEHOLDER] | [PLACEHOLDER] | **Export auditing and labelling done 2026-09-03** (C12/C13, Laravel only); `log_record_opens` default and the legacy system's unaudited exports remain open |
