...
- **Sessions and logs live in the container.** `SESSION_DRIVER=file` and `LOG_CHANNEL` write under `storage/`; a redeploy starts a fresh container, so every user is signed out (trusted-device MFA skips survive — they are in the database) and the previous container's log files are gone unless `storage/` is mounted as a persistent volume in Coolify (check the app's *Storages* tab). Ship logs elsewhere before relying on them for an investigation.
...
**Status (2026-09-02): GitHub Actions is billing-blocked** — jobs are created with zero steps and nothing executes, and `composer audit` in the backend job is `|| true`. Until billing is restored and that `|| true` removed, the workflow proves nothing; every release must clear the same gates **locally**, exactly as listed in [`RELEASE-CHECKLIST.md`](RELEASE-CHECKLIST.md). When CI is back: a red build means a genuine test regression, a contrast regression, allow-list drift, or an un-rebuilt `public/build` — none of these is neutral logging; the gates are the assertion.
