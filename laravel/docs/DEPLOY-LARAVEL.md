...
| Host | One OCI Ubuntu instance (region `me-riyadh-1`), reached as `ubuntu@<origin>` with the OCI SSH key. **SSH (port 22) is open to any address** (security-list rule; it was limited to the owner's workstation from 2026-09-23 to 2026-09-25 and reopened by owner decision because that address keeps changing). Login is key-only — password and keyboard-interactive login and root login are off in `sshd` — and fail2ban is active; the web console and serial console still work without SSH. The origin IP is deliberately not written here — it is what the Cloudflare proxy hides; it is in the private ops note. `ubuntu` has passwordless `sudo`; Docker commands need `sudo`. |
...
| Restore a current-size dump into a **scratch** database, then `audit:verify` against it | scratch DB, never `dmc_demo` | ☑ 2026-09-03 03:07 and 18:08 UTC, 2026-09-22 11:02 (`db-restore-drill.sh`, `COMPARE_LIVE=1`) | **7–8 s** end to end for a 20 MB dump (download 1 s, restore 6 s); counts matched live. `audit:verify` over a restored copy is now ☑ via the PITR rehearsal below (the drill itself still drops its scratch database on exit). |
| **Point-in-time recovery**: nightly dump + shipped binlogs replayed to a chosen moment (`BACKUP-AND-RESTORE.md` §10.5) | a **throwaway** MySQL server on an `--internal` network (`scripts/backup/pitr-rehearsal.sh`), never the production container | ☑ 2026-09-22 11:09 and 11:14 UTC (Claude Code, owner-instructed) | **50 s** end to end: base restore 6–7 s, replay of 02:15 → 10:30 (10 binlogs) 26–27 s. Recovered `audit_log` = **exactly** the 861 live rows before the stop time; `audit:verify` chain intact. The first attempt found that `mysqlbinlog` is **not in the `mysql:8` image** and that the runbook's chain check would have verified the live DB; both fixed (tools image `pitr-tools.Dockerfile`, one-off verify container) before the passing runs. |
...
- **Bootstrap-only in env (Coolify):** `APP_KEY`, `APP_ENV=production`, `APP_DEBUG=false`, `APP_URL`, `DB_*`, `LOG_CHANNEL=daily` / `LOG_LEVEL=warning`, `SESSION_SECURE_COOKIE=true`, `CSP_MODE=enforce`, `AUDIT_S3_*`. Leave `SESSION_EXPIRE_ON_CLOSE` unset — it defaults to `true` (the session ends when the browser closes; owner decision 2026-09-23) and `smoke.sh` fails if the live session cookie becomes persistent again. (No error-tracking SDK is installed yet — choosing one is an open owner item, `REMAINING-WORK.md` §B.) `APP_TIMEZONE=Asia/Riyadh` is the fallback for the in-app value below — date columns are timezone-naive and every "today" rule drifts by 3 h if the app runs in UTC.
...
- **To reload without disrupting sign-in ("keep MFA")**, snapshot → import → **verify counts** → restore, never in one step: (1) `CREATE TABLE _mfa_bak AS SELECT legacy_id, mfa_secret, mfa_recovery_codes, mfa_enrolled_at, email_verified_at, tour_completed_at FROM users WHERE legacy_id IS NOT NULL AND (mfa_enrolled_at IS NOT NULL OR email_verified_at IS NOT NULL)`; (2) run the import and compare its printed counts with the source; (3) `UPDATE users u JOIN _mfa_bak b ON u.legacy_id = b.legacy_id SET u.mfa_secret = b.mfa_secret, u.mfa_recovery_codes = b.mfa_recovery_codes, u.mfa_enrolled_at = b.mfa_enrolled_at, u.mfa_last_counter = b.mfa_last_counter, u.email_verified_at = b.email_verified_at, u.tour_completed_at = b.tour_completed_at;` (include `mfa_last_counter` in the step-1 snapshot too — it is the replay guard) then `TRUNCATE trusted_devices;` (stale device cookies would map onto re-seeded ids) and drop `_mfa_bak`; (4) **`TRUNCATE sessions;`** — sessions live in the database since 2026-09-03, and a surviving row would carry an old user id that now belongs to someone else. (The old instruction to delete `storage/framework/sessions/*` only applied to the file driver.) Put the app in maintenance mode (`php artisan down`) for the whole sequence and bring it `up` after the checks.
- **Rehearsed 2026-09-23** against a copy of production (BACKUP-AND-RESTORE.md §8): restore 7 s, import 18 s, 29 s end to end; counts equal the source, MFA and verified emails carried, settings byte-identical, audit chain intact, 52/52 figures reconcile. The import also clears `notifications` (by design). Nobody had changed their password in the new app, so the legacy password hashes the import brings back are everyone's current ones — re-check that count before the real run.
...
2. **Coolify application:** source = the GitHub repo, branch `main`, build pack Nixpacks, base directory `/laravel`, exposed port **8000**, domain `https://dmc-new.towardpcc.com` (proxied in Cloudflare, SSL mode *strict*). Health check on **`/login`** (what production uses — it exercises the database and sessions, so a broken image is never routed to), and the post-deployment `php artisan migrate --force` step (that is where §1 step 4 comes from). Nixpacks variables, build **and** runtime: `NIXPACKS_NODE_VERSION=22`, `NIXPACKS_PHP_ROOT_DIR=/app/public`, and **`NIXPACKS_PHP_FALLBACK_PATH=` present but empty** — without it the generated nginx config has a duplicate `location /` and the container never starts. The database must already have its schema (restored, or `migrate --force` from a one-off container) before the first deploy, because `/login` fails on an empty database. (All three found by the 2026-09-23 rebuild rehearsal, BACKUP-AND-RESTORE.md §5.1.)
...
- **Drain step on the container swap (RES-12) — applied 2026-09-03.** Coolify starts the new container, repoints Traefik and stops the old one; the question was how long a request in flight on the old container gets to finish. Verified in Coolify v4.1.2's source on 2026-09-03: the deployment stops the old container with `docker stop -t <grace>` where the grace comes from `application_settings.stop_grace_period` (the *Advanced* page) and, when that is unset, from Coolify's own default of **30 s** (`deploymentStopGracePeriodSeconds()` in `ApplicationDeploymentJob`; allowed range 1–3600 s). So the drain already existed by default; it is now set **explicitly to 30 s** on this application so a future Coolify default change cannot silently remove it. That is the whole drain under this image's process model, and it works precisely because of it: PID 1 in the Nixpacks container is `bash -c 'node prestart… && (php-fpm & nginx)'`, which does not forward `SIGTERM`, so for those 30 s nginx and PHP-FPM keep finishing whatever they were serving while no new requests arrive (Traefik has already moved), and only then does Docker `SIGKILL` the container. The `process_control_timeout` setting in `php-fpm.conf` recommended by earlier audits is **not applicable** here: PHP-FPM only drains on `SIGQUIT`, and no stop signal ever reaches it. Keep deploying in the agreed low-activity window and warn the clinical owner (§2); the grace period turns an instant cut into a 30-second finish.
...
- **The MySQL session timezone is deliberately NOT pinned (I18N-02) — and pinning it is a data migration, not a config change.** `config/database.php` sets no `timezone` on the `mysql` connection, so the DB session runs in the DB host's zone (UTC on this box) while the app runs Asia/Riyadh. Two guards keep that deliberate: `AppClockDayBoundaryTest::test_the_mysql_connection_pins_no_session_timezone` fails if the key appears, and `scripts/clock-guard.php` (a blocking CI gate) fails if new code reaches for MySQL's clock without an allow-listed reason in `.clock-allowlist.json`.
  - **Why not just pin it.** MySQL re-interprets every `TIMESTAMP` column through the session zone on **both** write and read. Adding `'timezone' => '+00:00'` changes only the *interpretation*, never the stored bytes, so on the next request every `created_at`, `assigned_at`, `mfa_enrolled_at`, `signoff_date` and audit row already in the database reads three hours away from what it meant when it was written — silently, with no error and no migration. On a hash-chained audit log that is not a display bug.
  - **What is safe today.** A DB-written column compared against the DB clock (`audit_log.created_at` is a `->useCurrent()` default, so both sides are the DB clock — the three allow-listed `NOW()` sites), and an app-written column compared against a value bound from PHP (`Carbon::today()`), which is how Dashboard / DataQuality / Registry were fixed. What breaks is mixing them: an app-written column against MySQL's `NOW()` / `CURDATE()`.
  - **If it is ever wanted** (a read replica, a BI tool or a restore on a differently-zoned host would want it): take the app down, dump, `UPDATE` every `TIMESTAMP` column by the offset in one transaction, set the config key, restore, and re-run `audit:verify` — the chain hashes the row values, so the audit table needs its chain re-baselined afterwards (§4.2 notes the same effect after a restore). Do it in one maintenance window with the pre-deploy dump in hand; there is no incremental path.
