...
|---|---|---|
| **Admit patient** (form on `/admissions/create`) | POST `/admissions` | INSERT `patients` (if the MRN is new) + INSERT `admissions` (location, bed, admit_date, admitted_from, `admitted_by`=you, `consultant_id`=NULL — or the consultant chosen on the form, which must be an **active consultant** since 2026-09-25; any other user is refused) + INSERT one `admission_diagnoses` row per ICD-10 picked. → `admission.create` | **Known MRN (since 2026-09-24):** if the submitted name/age/gender/nationality differ from a stored non-empty value, the request must carry `confirm_identity_update=true` or it is refused (422, naming the fields); once confirmed the `patients` row is updated (latest details win — for every episode). Filling a previously empty field needs no confirmation. The form looks the MRN up first — POST `/admissions/lookup-mrn` (JSON, MRN in the body, same gate as admitting, `throttle:phi`, → `patient.lookup` only while `log_record_opens` is on) — and prefills the stored details.
| **Admission from ICU** → "To ward" | POST `/admissions/{id}/icu-pull` | Gated by `can_add` (not `can_manage`); the ICU episode must still be active. Transaction: close the ICU episode (`discharge_date`=today, `transfer_type='Transfer from ICU'`, `discharge_to='Ward'`, `medical_discharge_date`=today, `outcome='Alive'`, `delay_reason`=NULL, `discharged_by`=you) + INSERT a new **unassigned** Ward admission (`consultant_id`=NULL, no bed, `admitted_from='ICU'`, diagnoses carried) so the patient re-enters the assignment queue, + void pending signatures on the closed episode. → `admission.icu_pull` *(verified against `PatientActionController::icuPull` on 2026-09-03 — the doc previously described this as `/transfer` carrying the consultant over)* | `is_longterm` carries over to the new ward episode (since 2026-09-24).
...
| **Assign to me** | POST `/admissions/{id}/assign-to-me` | UPDATE `consultant_id`=you + `is_new_assignment`, `assigned_on`, `assigned_at`. **Unassigned queue only** — an already-assigned patient must go through Assign/Transfer so the gate + signature apply. **Consultants only** since 2026-09-25 (owner decision; it was open to every clinical role) — holding the consultant slot lets the holder transfer and discharge the patient and lists them as a consultant, so everyone else names a consultant with Assign. Episodes already held by a non-consultant stay editable. → `admission.assign_to_me` |
...
| **Hand to a colleague** (consultant, own patient) | POST `/admissions/{id}/assign` | Since 2026-09-24 (owner decision): a consultant who is the current consultant of an ACTIVE episode may reassign it to another active consultant without `can_assign` (`User::canHandOffAdmission`); choosing yourself is refused. Same effects as Reassign — `consultant_id`, the handover signature for the receiver, their `handover.transfer` notification, the same-day reminder. → `admission.assign` with `via: consultant_hand_off` |
...
| **Undo sign-off** (Admin, same day) | POST `/consultations/{id}/reverse-signoff` | UPDATE clears the WHOLE closing entry — `signoff_date`, `signed_off_at`, `signed_off_by`, `response_disposition`, `response_followup_needed`, `response_note` — and returns `status='ongoing'` (back on the books, no daily commitment invented). The cleared response is preserved in the audit detail — disposition and follow-up flag as-is, the note only as `note_encrypted` (ciphertext under `APP_KEY`) plus `note_length`, never in the clear. Same-day only. → `consultation.reverse_signoff` |
...
| **Workspace / tabs / search** | GET `/consultations` · POST `/consultations/search` | Read only. Four status tabs, per-tab counts computed through the SAME visibility scope as the list, plus today's follow-up worklist. A row carrying a `signoff_date` is filed under **Signed off** whatever its `status` says (legacy drift), so no open tab or count includes it — the same rule as the consultation dashboard, the handover sheet and the Registry. The free-text term rides a POST body (SPC-TM-011). |
...
| **Data quality**, **Security panel**, **Orphan diagnoses** | GET `/data-quality` · `/security` · `/admin/orphan-diagnoses` | Read-only reports. No writes, not audited. The Security panel's MFA list (and the dashboard admin band's security count) covers **every** active unenrolled user — `mfa_enforcement` does not narrow it. |
...
| **Edit user** (role, active, on-service, specialty, capabilities incl. `can_coordinate_consultations`) | PUT `/control/users/{id}` | UPDATE `users` (guards against self-demotion; granting admin requires step-up). **Deactivating** also revokes the user's trusted devices and DELETEs their `sessions` rows, so a signed-in session ends at once. → `user.update` (field diff, plus `sessions_ended` on deactivation) |
| **Delete user** (step-up) | DELETE `/control/users/{id}` | **Soft delete** — sets `deleted_at`; you cannot delete your own account. Revokes trusted devices and DELETEs the user's `sessions` rows first. → `user.delete` (incl. `sessions_ended`) |
...
| **Add specialty** | POST `/control/specialties` | INSERT `specialties`; a name already used (case- and space-insensitive) is refused. → `specialty.add` |
| **Add indication** | POST `/control/reasons` | INSERT `consultation_reasons`; a duplicate name (case- and space-insensitive) is refused. → `reason.add` |
| **Rename / delete specialty** | PUT · DELETE `/control/specialties/{id}` | UPDATE `specialties.name` (same uniqueness rule) / DELETE — refused while any user or consultation references it, and always for id 1 (the Hospitalist pool the shuffle and census rely on). → `specialty.rename` (before/after) / `specialty.delete` |
| **Rename / delete indication** | PUT · DELETE `/control/reasons/{id}` | UPDATE `consultation_reasons.name` / DELETE — refused while any consultation's indications reference it, and always for id 0 ("Other", which requires free text). → `reason.rename` / `reason.delete` |
...
| **Session timeout** | (middleware) | An idle/absolute timeout logs the user out and writes `session.timeout`. Every request also re-checks that the account is still active and not deleted; if not, the user is signed out with a generic message and `session.evicted` (`reason`) is written. |
