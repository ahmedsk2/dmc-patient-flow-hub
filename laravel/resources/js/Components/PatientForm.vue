...
<template>
    <div class="grid grid-cols-2 gap-3">
        ...
        <div><label class="mb-1 block text-sm font-semibold text-ink-700">Gender</label><select v-model="form.gender" :class="fieldClass"><option value="">—</option><option>Male</option><option>Female</option></select><p v-if="form.errors.gender" class="mt-1 text-xs text-on-danger">{{ form.errors.gender }}</p></div>
        <div class="col-span-2"><label class="mb-1 block text-sm font-semibold text-ink-700">Nationality</label>
            ...
            <p v-if="form.errors.nationality" class="mt-1 text-xs text-on-danger">{{ form.errors.nationality }}</p></div>
        <div><label class="mb-1 block text-sm font-semibold text-ink-700">Admit date</label><input v-model="form.admit_date" type="date" :max="today" :class="[fieldClass, form.errors.admit_date && 'border-danger-500']" /><p v-if="form.errors.admit_date" class="mt-1 text-xs text-on-danger">{{ form.errors.admit_date }}</p></div>
        <div><label class="mb-1 block text-sm font-semibold text-ink-700">Location</label><select v-model="form.current_location" :class="fieldClass"><option>ER</option><option>Ward</option><option>ICU</option></select><p v-if="form.errors.current_location" class="mt-1 text-xs text-on-danger">{{ form.errors.current_location }}</p></div>
        <div class="col-span-2"><label class="mb-1 block text-sm font-semibold text-ink-700">Admitted from</label><input v-model="form.admitted_from" :list="dlId" placeholder="ER, Clinic, Referral…" :class="fieldClass" /><datalist :id="dlId"><option v-for="o in admitFromOptions" :key="o" :value="o" /></datalist><p v-if="form.errors.admitted_from" class="mt-1 text-xs text-on-danger">{{ form.errors.admitted_from }}</p></div>
        <div v-if="consultants" class="col-span-2"><label class="mb-1 block text-sm font-semibold text-ink-700">Consultant <span class="font-normal text-ink-400">(quiet change — no “New” badge)</span></label>
            ...
            <p v-if="form.errors.consultant_id" class="mt-1 text-xs text-on-danger">{{ form.errors.consultant_id }}</p></div>
    </div>
    <div>
