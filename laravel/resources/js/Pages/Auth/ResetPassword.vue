    ...
    <div class="flex min-h-screen items-center justify-center bg-app p-6">
        <div class="w-full max-w-sm">
            ...
            <form @submit.prevent="submit" class="space-y-3 rounded-2xl bg-card p-6 shadow-card ring-1 ring-line">
                <div><label class="mb-1 block text-sm font-semibold text-ink-700">Email</label><input v-model="form.email" type="email" :class="[field, form.errors.email && 'border-danger-500']" /><p v-if="form.errors.email" class="mt-1 text-xs text-on-danger">{{ form.errors.email }}</p></div>
                <div><label class="mb-1 block text-sm font-semibold text-ink-700">New password</label><input v-model="form.password" type="password" :class="[field, form.errors.password && 'border-danger-500']" /><PasswordMeter :password="form.password" @score="pwScore = $event" /><p v-if="form.errors.password" class="mt-1 text-xs text-on-danger">{{ form.errors.password }}</p></div>
