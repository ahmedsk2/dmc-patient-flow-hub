...
const uid = useId();
const fid = (name) => `forgot-password-${uid}-${name}`;
    ...
    <div class="flex min-h-screen items-center justify-center bg-app p-6">
        <div class="w-full max-w-sm">
            ...
            <form @submit.prevent="submit" class="rounded-2xl bg-card p-6 shadow-card ring-1 ring-line">
                ...
                <label :for="fid('email')" class="mb-1 block text-sm font-semibold text-ink-700">Username or email</label>
                <input :id="fid('email')" v-model="form.email" type="text" autocomplete="username" autofocus :aria-describedby="form.errors.email ? fid('email') + '-err' : undefined" class="w-full rounded-xl border border-ink-200 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20" :class="{ 'border-danger-500': form.errors.email }" />
