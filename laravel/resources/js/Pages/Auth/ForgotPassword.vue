    ...
    <div class="flex min-h-screen items-center justify-center bg-surface p-6">
        <div class="w-full max-w-sm">
            <div class="mb-6 flex flex-col items-center text-center">
                ...
                <p class="mt-1 text-sm text-ink-500">Enter your email and we'll send a reset link.</p>
            </div>
            <form @submit.prevent="submit" class="rounded-2xl bg-white p-6 shadow-card ring-1 ring-ink-100/60">
                ...
                <label class="mb-1 block text-sm font-semibold text-ink-700">Email</label>
                <input v-model="form.email" type="email" autofocus class="w-full rounded-xl border border-ink-200 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20" :class="{ 'border-danger-500': form.errors.email }" />
