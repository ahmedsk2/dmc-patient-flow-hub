    ...
    <div class="flex min-h-screen items-center justify-center bg-app p-6">
        <div class="w-full max-w-md">
            ...
            <form @submit.prevent="submit" class="space-y-3 rounded-2xl bg-card p-6 shadow-card ring-1 ring-line">
                ...
                <div><label class="mb-1 block text-sm font-semibold text-ink-700">Email <span class="text-danger-600" aria-hidden="true">*</span></label><input v-model="form.email" type="email" required :class="[field, form.errors.email && 'border-danger-500']" /><p v-if="form.errors.email" class="mt-1 text-xs text-danger-600">{{ form.errors.email }}</p></div>
                <div class="grid grid-cols-2 gap-3">
