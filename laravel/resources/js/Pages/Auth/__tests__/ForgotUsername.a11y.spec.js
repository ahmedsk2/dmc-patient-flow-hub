...
// UX-04 (engineering closeout): the forgot-username page's one control had a bare sibling <label>
// with no for/id pairing. Same gate shape as Auth/Login.a11y.spec.js.
...
import ForgotUsername from '@/Pages/Auth/ForgotUsername.vue';
...
const mountAttached = (props = {}) => {
    ...
    const w = mount(ForgotUsername, { props, attachTo: host });
...
describe('Auth/ForgotUsername — accessible names (UX-04)', () => {
    ...
    it('pairs the label with its input via for/id', () => {
        ...
        expect(labels.map((l) => l.text())).toEqual(['Email']);
        const control = document.getElementById(labels[0].attributes('for'));
    ...
    it('announces a server-side error through aria-describedby', () => {
        ...
        const w = mountAttached();
        const input = w.find('input[autocomplete="email"]');
