        ...
        <div class="mx-auto max-w-3xl space-y-6">
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <section class="rounded-2xl bg-white p-6 shadow-card ring-1 ring-ink-100/60">
                    ...
                    <div class="flex justify-center">
                        <img v-if="qr" :src="qr" alt="Two-factor QR code" class="rounded-xl ring-1 ring-ink-100" />
                    </div>
                    <p class="mt-4 text-xs text-ink-400">Can't scan? Enter this key manually:</p>
                    <code class="mt-1 block break-all rounded-lg bg-surface px-3 py-2 text-sm font-semibold tracking-wider text-ink-700">{{ secret }}</code>
                </section>
            ...
            <section class="rounded-2xl border-2 border-dashed border-warning-300 bg-warning-50/50 p-6">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    <button @click="copyCodes" class="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-ink-600 ring-1 ring-ink-200 hover:bg-ink-50">Copy all</button>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    <code v-for="c in recoveryCodes" :key="c" class="rounded-lg bg-white px-3 py-2 text-center text-sm font-semibold tracking-wider text-ink-700 ring-1 ring-ink-100">{{ c }}</code>
                </div>
            </section>
