    ...
    <AppLayout title="New Admissions">
        ...
        <div class="mb-5 flex flex-wrap items-center gap-3">
            ...
            <div class="ml-auto flex flex-wrap gap-2">
                ...
                <InfoTip v-if="canAssign && queue.length" label="Shuffle / auto-assign" text="Auto-balances the queue across active, on-service consultants only — anyone off-service or inactive is skipped." />
        ...
        <BaseModal :open="!!assigning" :title="'Assign consultant'" :subtitle="assigning ? `${assigning.name} · MRN ${assigning.mrn}` : ''" size="md" field-first :closable="false" @close="closeAssign">
            <form @submit.prevent="submitAssign" class="space-y-4">
                ...
                <label class="flex items-center gap-2 text-sm text-ink-600"><input type="checkbox" v-model="aForm.mark_new" class="rounded text-brand-700" /> Mark as new patient <span class="text-xs text-ink-400">(check to show the “New” badge)</span>
                    <InfoTip label="Mark as new patient" text="Ticked: shows the “New” badge and marks the assignment time. This dialog defaults it unticked; the board’s assign/reassign dialogs default it ticked instead." />
                </label>
