...
const props = defineProps({ year: Number, availableYears: Array, months: Array, totals: Object, avgLos: Number, topDx: Array, perConsultant: Array, generatedAt: String });
    ...
    <AppLayout title="Annual Report">
        ...
        <div class="report mx-auto max-w-[820px] rounded-2xl bg-white p-10 shadow-card ring-1 ring-ink-100/60 print:rounded-none print:shadow-none print:ring-0">
            ...
            <div class="mb-6 grid grid-cols-3 gap-3 sm:grid-cols-6">
                <div v-for="kpi in [['Admissions', totals.admissions],['Discharges', totals.discharges],['ICU', totals.icu],['Mortality', totals.deaths],['Mortality %', totals.mortalityRate + '%'],['Avg LOS', avgLos + 'd']]" :key="kpi[0]"
                    ...
                    <table class="w-full text-sm">
                        <tbody>
                            <tr v-for="c in perConsultant" :key="c.name" class="border-b border-ink-50">
                                ...
                                <td class="nums py-1.5 text-right font-semibold text-brand-700">{{ c.c }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
