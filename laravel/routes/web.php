...
Route::middleware(['auth', 'session.timeout', 'email.verify', 'mfa.enroll', 'pwd'])->group(function () {
    // PERF-08: per-user 'phi' throttle (240/min) on every PHI-bearing page and the JSON endpoints
    // those pages poll — see the limiter's comment in AppServiceProvider for why it's per-user, not
    // per-IP, and why 240/min never bites normal clinical use. Logout, /up, /health and /csp-report
    // are deliberately left unthrottled (session teardown / machine probes, no PHI).
    Route::get('/', [DashboardController::class, 'index'])->name('dashboard')->middleware('throttle:phi');
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard.alt')->middleware('throttle:phi');
    ...
    Route::get('/patients', [PatientsController::class, 'index'])->name('patients.index')->middleware('throttle:phi');
    ...
    Route::post('/patients', [PatientsController::class, 'index'])->name('patients.search')->middleware('throttle:phi');
    ...
    Route::post('/api/patients/quick-search', [PatientsController::class, 'quickSearch'])->name('patients.quickSearch')->middleware('throttle:phi');
    Route::get('/active-list', [PatientsController::class, 'activeList'])->name('patients.activeList')->middleware('throttle:phi');   // printable census (all roles, D1-scoped)
    ...
    Route::get('/admissions/{admission}/edit', [AdmissionsController::class, 'edit'])->middleware('throttle:phi')->name('admissions.edit');
    ...
    // Handovers — read is all-roles; save is canManage/outgoing (enforced in the action). The
    // 'phi' throttle sits on the two reads the board's bulk-reassign modal fetches as JSON
    // (this show route + preflight) and on the inbox/list pages; the SAVE write is left
    // unthrottled with the rest of the per-admission mutations below.
    Route::get('/admissions/{admission}/handover', [HandoverController::class, 'show'])->name('admissions.handover.show')->middleware('throttle:phi');
    Route::post('/admissions/{admission}/handover', [HandoverController::class, 'save'])->name('admissions.handover.save');
    Route::get('/handovers', [HandoverController::class, 'index'])->name('handovers.index')->middleware('throttle:phi');
    Route::get('/handovers/preflight', [HandoverController::class, 'preflight'])->name('handovers.preflight')->middleware('throttle:phi');
    ...
    // The bell dropdown AppLayout.vue fetches this on open/clear — not a fixed-interval poll, but
    // it rides on every authenticated page, so it shares the 'phi' bucket rather than the tighter one.
    Route::get('/api/notifications', [HandoverController::class, 'notifications'])->name('notifications.index')->middleware('throttle:phi');
    Route::post('/notifications/read-all', [HandoverController::class, 'readAll'])->name('notifications.readAll');
    Route::get('/consultations', [ConsultationsController::class, 'index'])->name('consultations.index')->middleware('throttle:phi');
    ...
    Route::post('/consultations/search', [ConsultationsController::class, 'index'])->name('consultations.search')->middleware('throttle:phi');
    ...
    Route::get('/consultations/dashboard', [ConsultationDashboardController::class, 'index'])->name('consultations.dashboard')->middleware('throttle:phi');
    ...
    Route::get('/consultations/handover', [ConsultationsController::class, 'handover'])->name('consultations.handover')->middleware('throttle:phi');
    ...
    Route::get('/recent', [RecentController::class, 'index'])->middleware('throttle:phi')->name('recent.index');

    // The unassigned-admission queue — same board character as /patients (lists name/MRN/diagnoses),
    // so it shares the 'phi' bucket. /create and the store POST show no existing patient's data.
    ...
    Route::middleware('admin')->group(function () {
        ...
        // PERF-08: the 'phi' page throttle sits on the screen/index views; the tighter 'phi-export'
        // bucket (20/min/user) sits on every CSV/XLSX/PDF download and report-generation endpoint
        // below — an export row is one patient, so exports get their own, much lower ceiling.
        Route::get('/statistics', [StatisticsController::class, 'index'])->name('statistics.index')->middleware('throttle:phi');
        // Phase 3 — §3.4: currently-filtered statistics exports (reuse the index query methods)
        Route::get('/statistics/export', [StatisticsController::class, 'exportXlsx'])->name('statistics.export.xlsx')->middleware('throttle:phi-export');
        Route::get('/statistics/export/pdf', [StatisticsController::class, 'exportPdf'])->name('statistics.export.pdf')->middleware('throttle:phi-export');
        Route::get('/registry', [RegistryController::class, 'index'])->name('registry.index')->middleware('throttle:phi');
        ...
        Route::post('/registry', [RegistryController::class, 'index'])->name('registry.search')->middleware('throttle:phi');
        // Phase 3 — §3.6: cheap match-count for the pre-export row-count advisory (before the exports)
        // — a JSON poll off the registry page, not itself an export, so it stays on the 'phi' bucket.
        Route::get('/registry/count', [RegistryController::class, 'matchCount'])->name('registry.count')->middleware('throttle:phi');
        Route::post('/registry/count', [RegistryController::class, 'matchCount'])->middleware('throttle:phi');
        Route::get('/registry/export', [RegistryController::class, 'export'])->name('registry.export')->middleware('throttle:phi-export');
        Route::post('/registry/export', [RegistryController::class, 'export'])->middleware('throttle:phi-export');
        Route::get('/registry/export-xlsx', [RegistryController::class, 'exportXlsx'])->name('registry.export.xlsx')->middleware('throttle:phi-export');
        Route::post('/registry/export-xlsx', [RegistryController::class, 'exportXlsx'])->middleware('throttle:phi-export');
        Route::get('/reports', [ReportsController::class, 'index'])->name('reports.index')->middleware('throttle:phi');
        Route::get('/reports/pdf', [ReportsController::class, 'pdf'])->name('reports.pdf')->middleware('throttle:phi-export');
        Route::get('/reports/monthly', [ReportsController::class, 'monthly'])->name('reports.monthly')->middleware('throttle:phi');
        Route::get('/reports/monthly/pdf', [ReportsController::class, 'monthlyPdf'])->name('reports.monthly.pdf')->middleware('throttle:phi-export');
        // Phase 3 — §3.6: stream a queued-generated (async) booklet PDF, deleted after send
        Route::get('/reports/pdf-download/{key}', [ReportsController::class, 'downloadGenerated'])->name('reports.pdf.download')->middleware('throttle:phi-export');
        // Phase 3 — §3.1: per-consultant scorecard PDF over a date range
        Route::get('/reports/consultant/{user}/pdf', [ReportsController::class, 'consultantPdf'])->name('reports.consultant.pdf')->middleware('throttle:phi-export');
        // Phase 3 — §3.2: governance / M&M pack (screen form + de-identified PDF)
        Route::get('/reports/governance', [ReportsController::class, 'governance'])->name('reports.governance')->middleware('throttle:phi');
        Route::get('/reports/governance/pdf', [ReportsController::class, 'governancePdf'])->name('reports.governance.pdf')->middleware('throttle:phi-export');
        ...
        // The viewer itself isn't in PERF-08's named PHI-page list (audit rows are actor/action/entity
        // metadata, not a patient board), but its CSV/XLSX exports are still bulk record exports, so
        // they share the tighter 'phi-export' bucket with every other export route.
        Route::get('/audit', [AuditController::class, 'index'])->middleware('throttle:phi')->name('audit.index');
        Route::get('/audit/export', [AuditController::class, 'export'])->name('audit.export')->middleware('throttle:phi-export');
        Route::get('/audit/export-xlsx', [AuditController::class, 'exportXlsx'])->name('audit.export.xlsx')->middleware('throttle:phi-export');

        // Phase 4 — Item 1: Recently Deleted (soft-delete) view + Restore actions
        Route::get('/trashed', [TrashedController::class, 'index'])->middleware('throttle:phi')->name('trashed.index');
        ...
        Route::get('/admin/orphan-diagnoses', [OrphanDiagnosesController::class, 'index'])->middleware('throttle:phi')->name('orphan.diagnoses');

        // Phase 4 — Item 6: data-quality dashboard
        Route::get('/data-quality', [DataQualityController::class, 'index'])->middleware('throttle:phi')->name('data-quality.index');
        ...
        Route::get('/admin/patient-merge', [PatientMergeController::class, 'index'])->middleware('throttle:phi')->name('patient-merge.index');
        // SPC-TM-011 (Wave 1): the merge typeahead searches by patient name/MRN — POST-only now.
        Route::post('/api/patients/search', [PatientMergeController::class, 'searchPatients'])->middleware('throttle:phi')->name('patient-merge.search-patients');
        Route::post('/admin/patient-merge/search', [PatientMergeController::class, 'search'])->middleware('throttle:phi')->name('patient-merge.preview');
