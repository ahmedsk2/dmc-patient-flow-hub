...
// var_dump($dishtransnumbers);
    ...
    // sub-query loop inside each month). Per admission only "does a qualifying prior record
    // exist" (0/1) matters, so EXISTS + COUNT(*) GROUP BY month reproduces the per-month
    // mysqli_num_rows(LIMIT 1) sums exactly.
    $gyear = date("Y", strtotime($date1));
    $gyStart = $gyear . '-01-01'; $gyEnd = $gyear . '-12-31';
    $readmissionByMonth = [];
    ...
    $stmt->bind_param('ss', $gyStart, $gyEnd);
    $stmt->execute();
    $rqr = $stmt->get_result();
    while ($row = $rqr->fetch_assoc()) { $readmissionByMonth[$row['ym']] = (int) $row['c']; }

        // Per-(month,consultant) LOS for the whole year in ONE grouped fetch (was an N-consultants query
    // loop PER month) + the 3 per-month LOS metrics as grouped-by-month maps. Day-diffs are exact
    // integers -> PHP averages are order-independent -> byte-identical to the per-month loops.
    $cLosByMC = [];
    $cLosStmt = $mysqli->prepare("SELECT consultant_id, MONTH(DISDATE) m, ADMDATE, DISDATE FROM picupatients WHERE DISDATE IS NOT NULL AND DISDATE BETWEEN ? AND ? AND (current_location != 'ICU' or current_location is null)");
    $cLosStmt->bind_param('ss', $gyStart, $gyEnd); $cLosStmt->execute();
    $cRes = $cLosStmt->get_result();
    while ($x = $cRes->fetch_assoc()) { $cLosByMC[(int) $x['m']][(int) $x['consultant_id']][] = abs(strtotime($x['ADMDATE']) - strtotime($x['DISDATE'])) / 86400; }
    $mLosAvg = function ($endCol, $extra) use ($mysqli, $gyStart, $gyEnd) {
        ...
        $st->bind_param('ss', $gyStart, $gyEnd); $st->execute();
        $r = $st->get_result(); $b = [];
    ...
    foreach($consultants as $c){
      $los = $cLosByMC[(int) $mdate1][(int) $c['member_id']] ?? [];
      $average = count($los) > 0 ? array_sum($los) / count($los) : 0;
      if ($average > 0) { array_push($consultantLOS, (number_format(($average), 1, '.', ''))); } else { array_push($consultantLOS, 0); }
      ...
      $n1 = 0;
      // Daily chart via 5 GROUP BY-day queries (was 5 x ~30 per-day queries). Per-day counts are
      // integers -> byte-identical to the per-day loop; days with no rows (incl. future days of the
      // current month) map to 0, exactly as before.
      $dayCount = function ($table, $col, $start, $end, $extra) use ($mysqli) {
          $st = $mysqli->prepare("SELECT DAY($col) d, COUNT(*) c FROM $table WHERE $col BETWEEN ? AND ?$extra GROUP BY d");
          $st->bind_param('ss', $start, $end); $st->execute();
          $r = $st->get_result(); $m = []; while ($x = $r->fetch_assoc()) { $m[(int) $x['d']] = (int) $x['c']; } return $m;
      };
      $nicu = " AND (current_location != 'ICU' or current_location is null)";
      $admMap = $dayCount('picupatients', 'ADMDATE', $first_day_ofmonth, $last_day_ofmonth, $nicu);
      $disMap = $dayCount('picupatients', 'DISDATE', $first_day_ofmonth, $last_day_ofmonth, $nicu);
      $conMap = $dayCount('consultations', 'consultation_date', $first_day_ofmonth, $last_day_ofmonth, '');
      $sgnMap = $dayCount('consultations', 'signoff_date', $first_day_ofmonth, $last_day_ofmonth, '');
      $icuMap = $dayCount('picupatients', 'DISDATE', $first_day_ofmonth, $last_day_ofmonth, " AND DISTO = 'Intensive Care (ICU)'");
      while ($n1 < $ds) {
          array_push($label, $n1);
          array_push($admissions, $admMap[$n1 + 1] ?? 0);
          array_push($discharges, $disMap[$n1 + 1] ?? 0);
          array_push($toicu, $icuMap[$n1 + 1] ?? 0);
          array_push($newconsults, $conMap[$n1 + 1] ?? 0);
          array_push($signedoff, $sgnMap[$n1 + 1] ?? 0);
          $n1++;
      }
      // Preserve the original per-day loop's leftover: after it ran, $dischargedpcount held the
      // LAST day's discharge count. It's used only as the zero-guard at the "Weekend Discharge"
      // counter below (the % there divides by array_sum($discharges)), so reproduce it exactly.
      $dischargedpcount = $disMap[$ds] ?? 0;
      //////////////
      // Medical / physical / ICU LOS — grouped month-map lookups (built once above the loop)
      $mi = (int) $mdate1;
...
// equivalent to the per-day census for every month incl. the current partial one on the full
// production dataset (54/54 months, 0 mismatches). ($ds remains set above for the daily chart.)
...
if (strtotime($cap) < strtotime($first_day_ofmonth)) {
    $allweekend_discharge = 0; // wholly-future month
} else {
...
<page size="A4" layout="landscape">
  ...
  <div class="row">
    <div class="col-md-12" style=" text-align: center; margin-top: 15%; ">
      <spnan style=" color: #004aab; font-size: xx-large; font-weight: 600; font-family: inherit; ">Internal Medicine Department Performance Report For <?php echo (int)$ydate1 . "/" . (int)$mdate1; ?></spnan>
    </div>
  </div>
  </page>
